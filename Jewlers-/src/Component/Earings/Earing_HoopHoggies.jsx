...
function Earing_HoopHuggies() {
  ...
  useEffect(() => {
    const fetchEarrings = async () => {
      ...
      try {
        const response = await fetch(`${backendUrl}/api/products/Earrings/Hoop%20&%20Huggies`);
        if (!response.ok) {
  ...
  return (
    <>
      <h1 className="text-black lg:text-2xl font-bold p-2 lg:ml-8 my-3 md:text-left text-center">
        Hoop & Huggies ({filteredEarrings.length})
      </h1>
...
export default Earing_HoopHuggies;
