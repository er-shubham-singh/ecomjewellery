...
function Ring_DiamondEngagement() {
    ...
    const fetchRings = async () => {
      ...
      try {
        const response = await fetch(
          `${backendUrl}/api/products/Rings/Diamond%20Engagement%20Rings`
        )
        if (!response.ok) {
  ...
  return (
    <>
      <h1 className="text-black lg:text-2xl font-bold p-2 lg:ml-8 my-3 md:text-left text-center">
        Diamond Engagement Rings ({filteredRings.length})
      </h1>
...
export default Ring_DiamondEngagement
