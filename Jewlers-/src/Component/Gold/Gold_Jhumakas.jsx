...
function Gold_Jhumakas() {
  ...
  useEffect(() => {
    ...
    fetch(`${backendUrl}/api/products/Gold/Jhumkas`)
      .then((response) => {
        if (!response.ok) {
  ...
  const handleSort = (order) => {
    setSortOrder(order);
    setLoading(true); // Show loading while sorting
  ...
  return (
    <>
      <h1 className="text-black lg:text-2xl font-bold p-2 lg:ml-8 my-3 md:text-left text-center">
        Gold Jhumakas ({filteredGold.length})
      </h1>
