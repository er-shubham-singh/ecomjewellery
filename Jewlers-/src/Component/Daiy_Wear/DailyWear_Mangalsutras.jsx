...
function DailyWear_Mangalsutras() {
  const [mangalsutrasData, setMangalsutrasData] = useState([]);
  const [filteredMangalsutras, setFilteredMangalsutras] = useState([]);
  ...
  useEffect(() => {
    const fetchMangalsutras = async () => {
      ...
      try {
        const response = await fetch(`${backendUrl}/api/products/daily%20wear/mangalsutras`);
        if (!response.ok) {
        ...
        setMangalsutrasData(products);
        setFilteredMangalsutras(products);
      } catch (error) {
  ...
  const handleSort = (option) => {
    ...
    setTimeout(() => {
      let sortedData = [...mangalsutrasData];
      ...
      setFilteredMangalsutras(sortedData);
      setLoading(false);
    }, 500);
  };
  ...
  return (
    <>
      <h1 className="text-black lg:text-2xl font-bold p-2 lg:ml-8 my-3 md:text-left text-center">
        Daily Wear Mangalsutras ({filteredMangalsutras.length})
      </h1>
      ...
      {loading ? (
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredMangalsutras.map((info, index) => (
            <ProductCard
              key={index}
...
export default DailyWear_Mangalsutras;
