...
function Dailywear_Chains() {
  ...
  useEffect(() => {
    const fetchChains = async () => {
      ...
      try {
        const response = await fetch(`${backendUrl}/api/products/daily%20wear/Chains`);
        if (!response.ok) {
        ...
        setChainsData(products);
        setFilteredChains(products);
      } catch (error) {
  ...
  return (
    <>
      <h1 className="text-black lg:text-2xl font-bold p-2 lg:ml-8 my-3 md:text-left text-center">
        Daily Wear Chains ({filteredChains.length})
      </h1>
