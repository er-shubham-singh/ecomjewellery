...
function AllJewelry_Mangalsutras() {
    ...
    const fetchProducts = async () => {
      ...
      try {
        const response = await axios.get(
          `${backendUrl}/api/products/all%20jewellery/mangalsutras`
        );
  ...
  return (
    <>
      <h1 className="text-black lg:text-2xl font-bold p-2 lg:ml-8 my-3 md:text-left text-center">
        Mangalsutras ({filteredProducts.length})
      </h1>
      ...
      {loading || filterLoading ? (
        ...
        <p className="text-center text-gray-500">No mangalsutras available</p>
      ) : (
