...
function Diamond_Bracelets() {
  ...
  useEffect(() => {
    const fetchDiamonds = async () => {
      ...
      try {
        const response = await fetch(`${backendUrl}/api/products/Diamond/Bracelets`);
        if (!response.ok) {
  ...
  return (
    <>
      <h1 className="text-black lg:text-2xl font-bold p-2 lg:ml-8 my-3 md:text-left text-center">
        Diamond Bracelets ({filteredDiamond.length})
      </h1>
...
export default Diamond_Bracelets;
