...
function Diamond_Pendants() {
  ...
  useEffect(() => {
    const fetchPendants = async () => {
      ...
      try {
        const response = await fetch(`${backendUrl}/api/products/Diamond/Pendants`);
        if (!response.ok) {
  ...
  return (
    <>
      <h1 className="text-black lg:text-2xl font-bold p-2 lg:ml-8 my-3 md:text-left text-center">
        Diamond Pendants ({filteredDiamond.length})
      </h1>
...
export default Diamond_Pendants;
